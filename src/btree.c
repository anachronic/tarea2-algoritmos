 ...
 * un nodo en memoria secundaria es 4092 (sobran 4 bytes que rellenaremos con 0s).
 *
...
char *serializar_nodo(struct btree_nodo *b) {
  ...
  for (k = 0; k < num_elems; k++) {
    memcpy(buffer + 2 * sizeof(int) + k * TAMANO_CADENA * sizeof(char), b->elementos[k], TAMANO_CADENA);
  }

  // hasta aquí tenemos la cant. de elementos, todas las palabras y sólo faltan los hijos
  // ie: tenemos 4 + BTREE_ELEMS_NODO * 16 bytes.
  memcpy(buffer + 2 * sizeof(int) + BTREE_ELEMS_NODO * TAMANO_CADENA * sizeof(char),
...
struct btree_nodo *deserializar_nodo(char *buffer) {
  ...
  for (k = 0; k < nodo->num_elems; k++) {
    ...
    memcpy(nodo->elementos[k], buffer + 2 * sizeof(int) + k * TAMANO_CADENA * sizeof(char), TAMANO_CADENA);
  }

  // finalmente, copiamos los indices de los hijos.
  nodo->hijos = (int *) malloc(sizeof(int) * (BTREE_ELEMS_NODO + 1));
  memcpy(nodo->hijos, buffer + 2 * sizeof(int) + BTREE_ELEMS_NODO * TAMANO_CADENA * sizeof(char),
